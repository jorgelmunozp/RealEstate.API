    ...
    public class PropertyModelValidator : AbstractValidator<PropertyModel>
    {
        public PropertyModelValidator()
        {
            RuleFor(p => p.Id).NotEmpty()
                .WithMessage("El Id de la propiedad es obligatorio");
            ...
            RuleFor(p => p.CodeInternal).GreaterThan(0)
                .WithMessage("El código interno debe ser mayor a 0");

            RuleFor(p => p.Year).InclusiveBetween(1800, DateTime.Now.Year)
                .WithMessage($"El año debe estar entre 1800 y {DateTime.Now.Year}");

            RuleFor(p => p.IdOwner).NotEmpty()
                .WithMessage("El Id del propietario es obligatorio");
        }
